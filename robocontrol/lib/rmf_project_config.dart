...
String buildFleetAdapterYaml({
  ...
  final buffer = StringBuffer()
    ...
    ..writeln('# robocontrol 가 맵 프로젝트에서 생성했다. 손으로 고치면')
...
String buildFleetSimYaml({
  ...
  final buffer = StringBuffer()
    ...
    ..writeln('# robocontrol 가 맵 프로젝트에서 생성했다.')
    ..writeln('#')
...
String buildProjectRunScript({
  ...
  int rosDomainId = defaultRosDomainId,
    ...
    '''#!/usr/bin/env bash
# $mapName 프로젝트 실행.
# robocontrol 가 맵 프로젝트에서 생성했다. 손으로 고치면 다음 저장 때
# 덮어써진다.
#
...
watch_map_server &
watch_fleet_adapter &
# Nav2 단독 시작 버튼과 전체 프로젝트 실행이 같은 잠금을 쓴다. 이미 한 세트가
# 떠 있으면 두 번째 세트를 만들지 않고 기다리며, 기존 세트가 끝난 뒤에만 이어
# 받는다. 여러 controller_server가 같은 cmd_vel을 발행하는 일을 막는다.
...
# robocontrol 가 맵 프로젝트에서 생성했다. 손으로 고치면 다음 배포 때
# 덮어써진다.
#
...
String buildRobotNav2LaunchXml(
  ...
  final buffer = StringBuffer()
    ...
    ..write('    <push-ros-namespace namespace="$namespace"/>\n')
    ..writeln('')
...
def main(argv=sys.argv):
    ...
    request = ApiRequest()
    request.request_id = 'robocontrol-' + str(uuid.uuid4())[:8]
...
String buildRobotReadme(
  ...
  final buffer = StringBuffer()
    ...
    ..writeln('robocontrol 가 로봇 등록에서 만들었습니다.')
    ..writeln()
    ..writeln('| 항목 | 값 |')
    ..writeln('|---|---|')
