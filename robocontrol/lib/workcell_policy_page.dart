...
import 'external_python_runner.dart';
...
class _WorkcellPolicyPageState extends State<WorkcellPolicyPage> {
    ...
    builder: (_) => AlertDialog(
      ...
      content: SizedBox(
        ...
        child: SelectableText(
          ...
          '실행 시간: ${(result.duration.inMilliseconds / 1000).toStringAsFixed(1)}초\n\n'
          '${result.output.isEmpty ? '출력이 없습니다.' : result.output}',
        ),
      ),
      actions: [
        FilledButton(
  ...
  Future<void> _runExternalFile() async {
    final picked = await FilePicker.platform.pickFiles(
      dialogTitle: '실행할 Python 파일 선택',
      type: FileType.custom,
      allowedExtensions: const ['py'],
    );
    final path = picked?.files.single.path;
    if (path == null || !mounted) return;
    final draft = await showMovableDialog<_ExternalRunDraft>(
      context: context,
      builder: (_) => _ExternalPythonRunDialog(path: path),
    );
    if (draft == null || !mounted) return;
    setState(() => _executing = path);
    final result = await runExternalPython(
      ExternalPythonRequest(
        filePath: path,
        arguments: splitCommandArguments(draft.arguments),
        timeout: Duration(seconds: draft.timeoutSeconds),
      ),
    );
    if (!mounted) return;
    setState(() => _executing = null);
    await _showExecutionResult(
      result.success ? 'Python 실행 완료' : 'Python 실행 실패',
      result,
    );
  }

  Future<void> _runPolicy(WorkcellPolicy policy) async {
    final project = widget.projectName;
    if (project == null || policy.archiveMissing) return;
    final arms = widget.robots.where((robot) => !robot.isMobile).toList();
    final pinkies = widget.robots.where((robot) => robot.isMobile).toList();
    if (arms.isEmpty) {
      _message('등록된 로봇팔이 없습니다. 로봇 관리에서 먼저 등록하세요.');
      return;
    }
    final draft = await showMovableDialog<_PolicyRunDraft>(
      context: context,
      builder: (_) =>
          _PolicyRunDialog(policy: policy, arms: arms, pinkies: pinkies),
    );
    if (draft == null || !mounted) return;
    setState(() => _executing = policy.id);
    final arm = arms.firstWhere((robot) => robot.robotId == draft.armId);
    final pinky = pinkies
        .where((robot) => robot.robotId == draft.pinkyId)
        .firstOrNull;
    final result = await runRobotArmPolicy(
      RobotArmPolicyRequest(
        runnerPath: projectPolicyRunnerPath(project),
        policyPath: policyArchivePath(policy),
        policyId: policy.id,
        armNamespace: arm.gzName,
        pinkyNamespace: pinky?.gzName ?? '',
        rosDomainId: arm.rosDomainId ?? pinky?.rosDomainId ?? 12,
        robotModel: policy.robotModel,
        runSeconds: draft.seconds,
      ),
    );
    if (!mounted) return;
    setState(() => _executing = null);
    await _showExecutionResult(
      result.success ? '${policy.id} 실행 완료' : '${policy.id} 실행 실패',
      result,
    );
  }
          ...
          Row(
            children: [
              ...
              Wrap(
                ...
                children: [
                  OutlinedButton.icon(
                    onPressed: _executing == null ? _runExternalFile : null,
                    icon: const Icon(Icons.code),
                    label: const Text('외부 Python 실행'),
                  ),
                  OutlinedButton.icon(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, index) {
                  ...
                  return PolicyListTile(
                    ...
                    running: _executing == policy.id,
                    onRun: !policy.archiveMissing && mine && _executing == null
                        ? () => unawaited(_runPolicy(policy))
                        : null,
            ...
            children: [
              if (onRun != null || running)
                FilledButton.icon(
                  ...
                  label: Text(running ? 'Policy 실행 중' : '로봇팔 Policy 실행'),
                ),
              if (onDownload != null)
                FilledButton.icon(
...
class _ExternalRunDraft {
  const _ExternalRunDraft(this.arguments, this.timeoutSeconds);
  final String arguments;
  final int timeoutSeconds;
}

class _ExternalPythonRunDialog extends StatefulWidget {
  const _ExternalPythonRunDialog({required this.path});
  final String path;

  @override
  State<_ExternalPythonRunDialog> createState() =>
      _ExternalPythonRunDialogState();
}

class _ExternalPythonRunDialogState extends State<_ExternalPythonRunDialog> {
  final _arguments = TextEditingController();
  final _timeout = TextEditingController(text: '300');
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: '실행 인자',
              hintText: '--policy pickup --station pick1',
              helperText: '공백이 포함된 값은 따옴표로 묶으세요.',
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ...
          if (timeout == null || timeout < 1) return;
          Navigator.pop(context, _ExternalRunDraft(_arguments.text, timeout));
        },
...
class _PolicyRunDraft {
  const _PolicyRunDraft({
    required this.armId,
    required this.pinkyId,
    required this.seconds,
  });
  final String armId;
  final String? pinkyId;
  final double seconds;
}

class _PolicyRunDialog extends StatefulWidget {
  const _PolicyRunDialog({
    required this.policy,
    required this.arms,
    required this.pinkies,
  });
  final WorkcellPolicy policy;
  final List<RmfProjectRobot> arms;
  final List<RmfProjectRobot> pinkies;

  @override
  State<_PolicyRunDialog> createState() => _PolicyRunDialogState();
}

class _PolicyRunDialogState extends State<_PolicyRunDialog> {
  late String _armId = widget.arms.first.robotId;
  late String? _pinkyId = widget.pinkies.firstOrNull?.robotId;
  final _seconds = TextEditingController(text: '6');
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    title: Text('${widget.policy.id} 실행'),
    content: SizedBox(
      width: 560,
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          DropdownButtonFormField<String>(
            ...
            items: [
              for (final robot in widget.arms)
                DropdownMenuItem(
                  value: robot.robotId,
                  child: Text('${robot.displayName} · ${robot.gzName}'),
                ),
            ],
            onChanged: (value) => setState(() => _armId = value!),
          ),
          const SizedBox(height: 12),
          DropdownButtonFormField<String?>(
            initialValue: _pinkyId,
            decoration: const InputDecoration(
              labelText: '먼저 정지시킬 Pinky',
              helperText: '선택하면 Policy 전에 cmd_vel 0을 두 번 보냅니다.',
              border: OutlineInputBorder(),
            ),
            items: [
              const DropdownMenuItem(value: null, child: Text('선택 안 함')),
              for (final robot in widget.pinkies)
                DropdownMenuItem(
                  value: robot.robotId,
                  child: Text('${robot.displayName} · ${robot.gzName}'),
                ),
            ],
            onChanged: (value) => setState(() => _pinkyId = value),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: _seconds,
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ...
          Navigator.pop(
            context,
            _PolicyRunDraft(armId: _armId, pinkyId: _pinkyId, seconds: seconds),
          );
        },
        icon: const Icon(Icons.play_arrow_rounded),
        label: const Text('정지 후 실행'),
      ),
    ],
  );
}
