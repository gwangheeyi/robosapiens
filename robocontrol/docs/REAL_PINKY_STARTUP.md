...
> **앱에서 브링업을 띄울 수 있다.** 로봇 등록에 SSH 주소를 넣어 두면 3절을
> 단추 하나로 대신하고, `켤 때 자동 실행` 을 걸면 0절의 순서 문제가 아예
> 사라진다 — 로봇 전원이 곧 브링업이 된다.
> [ROBOT_SSH_BRINGUP.md](ROBOT_SSH_BRINGUP.md) 를 보라.
>
> 앱은 백엔드를 띄운 뒤 7절의 확인과 되살리기를 **스스로 한다**. 아래 절차는
> 손으로 할 때와 무엇이 일어나는지 알고 싶을 때 본다.
...
```bash
stat -c '%y  %n' build/linux/x64/release/bundle/robocontrol lib/rmf_project_runner_io.dart
```
