...
import 'package:robocontrol/robot_telemetry_models.dart';
...
void main() {
  ...
  group('백엔드가 오르내려도 다시 붙는다', () {
    ...
    // 멀쩡한데 화면만 조용했다.
    test('RMF fleet_states 없이 로봇별 odom을 직접 읽는다', () {
      final source = File(
        'lib/robot_telemetry_bridge_io.dart',
      ).readAsStringSync();
      expect(source, isNot(contains('exec ros2 topic echo /fleet_states')));
      expect(source, isNot(contains('_openFleetStates')));
      expect(source, contains(r'exec ros2 topic echo ${feed.topic}'));
    });
