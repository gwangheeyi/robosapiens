...
- 일반: 경로 연결 및 일시 정지가 가능한 지점이며 `is_holding_point`로 내보냅니다.
- 대기: 작업이 없을 때 로봇을 장시간 세워 두는 전용 자리이며 `is_parking_spot`으로 내보냅니다.
- 홈
- 충전
- 픽업
- 드랍오프
...
역할만 추가하므로 같은 시설을 중복 생성하지 않습니다. Home 수와 같은 수의 대기
Waypoint를 별도로 확보하며 충전소는 반드시 충전 카테고리로 지정합니다. 모든 신규
Waypoint는 Floor 다각형 내부에만 생성됩니다.
...
입구 가까이에 대기 지점을 배치합니다. 단, 그 1경로가 넓어 왕복 분리된 경우에는
입구 대기 지점을 두지 않습니다. Home은 드랍오프와 가까운 후보를 우선
...
`drop/드랍/하차`, `pickup/픽업/상차`, `parking/holding/wait/대기`를 해당 운영
카테고리로 인식합니다. 고정 설비 카테고리는 이름으로 변경하지 않습니다.
