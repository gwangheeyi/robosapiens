...
/// 로봇이 그 자리에 **서는 자세**가 뜻을 갖는 자리인가.
///
/// 픽업·드랍오프는 팔이 수납함에 닿아야 해서 방향이 결과를 가른다.
/// 충전도 같다 — 충전 단자는 로봇의 한쪽에만 있으므로, 아무 쪽이나 보고 서면
/// 도착은 했는데 물리적으로 충전이 안 된다. 스폰 자세(`spawn_heading`)와 복귀
/// 자세가 갈리는 것도 여기서 온다: 자리 자체가 방향을 안 가지고 있으면 사람이
/// 로봇 등록에 따로 넣은 각도와 어긋나도 아무도 모른다.
/// 대기도 쓴다. 처음에는 "로봇이 **가는** 자리지 서 있는 자리가 아니다" 로
/// 보고 뺐는데, 실제로는 대기 자리를 목적지로 삼는다. 좁은 곳에서는 거기
/// 서는 방향이 다음 동작을 가른다 — 대기1 에서 -45도로 선 뒤 후진으로 픽업에
/// 들어가는 식이다. 방향이 없으면 들어온 길 방향대로 서므로 그 다음이 안 된다.
///
/// 주차는 아직 뺀다. 오래 세워 두는 자리라 방향이 뜻을 가질 수 있지만, 지금
/// 쓰는 자리가 아니다 — 쓰게 될 때 열면 된다.
...
///
/// 방향을 쓰지 않는 자리에 각도를 남겨 두면, 나중에 다시 픽업·드랍오프·충전으로
/// 바꿨을 때 잊어버린 옛 각도가 되살아난다. 그래서 지우는 것이 맞다 — 다만
/// **말없이** 지우면 안 된다.
///
