...
#   GAZEBO_GUI=true RVIZ=true ./run_project1-ver2.sh
is_true() {
  case "${1,,}" in
...
BUILDING_YAML="$MAP_DIR/project1-ver2.building.yaml"
...
LOG_FILE="$MAP_DIR/project1-ver2.log"
ERR_FILE="$MAP_DIR/project1-ver2.err.log"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "project1-ver2 프로젝트가 이미 실행 중입니다." >&2
  exit 1
fi
: > "$ERR_FILE"
...
echo "=== $(date '+%Y-%m-%d %H:%M:%S') project1-ver2 실행 ==="
...
ensure_world_sensors "$MAP_DIR/project1-ver2.world"
...
ensure_world_collision_detector "$MAP_DIR/project1-ver2.world"
...
echo "[2/3] Open-RMF"
ros2 launch "$MAP_DIR/project1-ver2.launch.xml" headless:="$RVIZ_HEADLESS" &
sleep 12
