#!/usr/bin/env python3
"""rosapiens 프로젝트의 작업 다리 — 앱이 만든 작업을 RMF 에 넣는다.

rmf_control_ui 가 맵 프로젝트에서 생성했다. 손으로 고치면 다음 저장 때
덮어써진다.

  rosapiens_task_bridge.py --submit 작업.json
...
def main(argv=sys.argv):
    parser = argparse.ArgumentParser(prog='rosapiens' + '_task_bridge')
    ...
    node = rclpy.node.Node('rosapiens_pinky' + '_task_bridge')
