...
if [[ -n "${ROS_DOMAIN_ID:-}" && "$ROS_DOMAIN_ID" != "12" ]]; then
  ROS_DOMAIN_SOURCE="환경 변수 — 프로젝트 설정 12 을 덮었습니다"
else
  ROS_DOMAIN_SOURCE="프로젝트 설정"
fi
export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-12}"

MAP_DIR="${MAP_DIR:-/home/gyi/robosapiens/rmf_maps/rosapiens}"
...
#   GAZEBO_GUI=true RVIZ=true ./run_rosapiens.sh
is_true() {
  case "${1,,}" in
...
BUILDING_YAML="$MAP_DIR/rosapiens.building.yaml"
...
LOG_FILE="$MAP_DIR/rosapiens.log"
ERR_FILE="$MAP_DIR/rosapiens.err.log"
LOG_MAX_MB="${LOG_MAX_MB:-200}"

# UI의 두 실행 경로가 거의 동시에 눌려도 두 번째 스크립트가 기존 로그를
# 비우거나 같은 월드를 한 벌 더 띄우지 못하게 한다. 잠금 FD는 이 셸이 끝날
# 때까지 유지된다.
LOCK_FILE="$MAP_DIR/.rosapiens.run.lock"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "rosapiens 프로젝트가 이미 실행 중입니다." >&2
  exit 1
fi
: > "$ERR_FILE"
...
echo "=== $(date '+%Y-%m-%d %H:%M:%S') rosapiens 실행 ==="
...
# 실제 PGID 를 적어야 한다.
PGID_FILE="$MAP_DIR/.rosapiens.pgid"
...
ensure_world_sensors "$MAP_DIR/rosapiens.world"
...
ensure_world_collision_detector "$MAP_DIR/rosapiens.world"
...
start_simulator() {
  case "$SIM_BACKEND" in
    gazebo)
      ...
      ros2 launch "$MAP_DIR/rosapiens_bringup.launch.xml" headless:="$GAZEBO_HEADLESS" &
      if ! wait_for_gazebo "$MAP_DIR/rosapiens.world"; then
    ...
    isaac_sim)
      ...
      local script="${ISAAC_PROJECT_SCRIPT:-$MAP_DIR/isaac/start_rosapiens.py}"
      local converter="${ISAAC_PROJECT_CONVERTER:-$MAP_DIR/isaac/convert_rosapiens.py}"
      local stage="${ISAAC_STAGE:-$MAP_DIR/isaac/rosapiens.usd}"
      if [[ ! -x "$launcher" ]]; then
      ...
      local rebuild=false
      [[ ! -f "$stage" || "$MAP_DIR/rosapiens.world" -nt "$stage" ]] && rebuild=true
      local generator
...
echo "[2/3] Open-RMF"
ros2 launch "$MAP_DIR/rosapiens.launch.xml" headless:="$RVIZ_HEADLESS" &
sleep 12
...
EXPECTED_FLEET_ROBOTS="pinky_01 pinky_04 pinky_05 pinky_06"
...
watch_map_server() {
  ...
  echo "지도 파일을 확인하세요: $MAP_DIR/nav2_map/rosapiens.yaml" >&2
  echo "오류만 모은 것: $ERR_FILE" >&2
  return 1
}

watch_fleet_adapter() {
  local pattern="$MAP_DIR/rosapiens_nav2_adapter.py"
...
watch_map_server &
watch_fleet_adapter &
ros2 launch "$MAP_DIR/rosapiens_nav2.launch.xml"
