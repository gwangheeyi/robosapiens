...
ROBOT_NAMESPACES="pinky_01 omx_01 omx_02 pinky_04 pinky_05 pinky_06"
...
sweep_map_dir() {
  ...
  while read -r pid; do
    ...
    [[ "$args" == *"stop_rosapiens.sh"* ]] && continue
...
stop_matching "Open-RMF (rosapiens)" "ros2 launch $MAP_DIR/rosapiens.launch.xml"
stop_matching "Gazebo bringup (rosapiens)" \
  "ros2 launch $MAP_DIR/rosapiens_bringup.launch.xml"
stop_matching "Gazebo 서버 (rosapiens)" "gz sim.*$MAP_DIR/rosapiens.world"
...
sweep_map_dir "이 맵을 물고 남은 노드 (rosapiens)"
...
sweep_namespaces() {
  ...
  if ((${#pids[@]} == 0)); then
    echo "로봇 네임스페이스 (rosapiens): 남은 것 없음"
    return
  fi
  stop_pids "로봇 네임스페이스 (rosapiens)" "${pids[@]}"
}
...
sweep_lock_holders() {
  ...
  for pass in 1 2 3; do
    ...
    ((${#pids[@]} == 0)) && return
    stop_pids "실행 잠금 보유 프로세스 (rosapiens, 확인 $pass)" "${pids[@]}"
  done
}
...
if ! flock -n "$LOCK_FILE" true; then
  echo "오류: rosapiens 실행 잠금이 아직 사용 중입니다." >&2
...
if [[ -n "$remaining_zombies" ]]; then
  echo "오류: rosapiens 관련 좀비 프로세스가 남았습니다:" >&2
...
echo "rosapiens 프로젝트 프로세스를 정리했습니다."
