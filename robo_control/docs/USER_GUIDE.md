...
| **1× 2× 4× 8×** | 시뮬레이션 배속. 기본 4×(실제 1초 = 시뮬레이션 4초). 배터리·처리량 변화를 빨리 보려면 8×로 올리세요. **현장 로봇이 접속해 있으면 1×로 잠깁니다**(자물쇠 아이콘). 실장비는 실시간으로 움직이기 때문입니다. |
...
그 아래 **현장 링크**는 실장비(Gazebo 시뮬레이터 포함)의 접속 상태입니다.

- `게이트웨이 대기 :8788` — 열려 있고 접속한 로봇 없음
- `실장비 n대 접속` — n대가 관제 제어를 받고 있음
- `게이트웨이 닫힘` — 포트를 열지 못함(다른 프로그램이 쓰는 중). 시뮬레이션
  로봇만으로 운영됩니다
...
| 노란 점 | 현재 점유 중인 랙 슬롯 |
| 주황 원 | **적재 스테이션**(LOAD-A·C·F). 구획마다 하나, 로봇팔이 화물을 실어 준다 |
| 큰 반투명 원 | 안전 상황 영향 반경 |
| 어두워진 구획 | 전원이 차단된 구획 |
...
상태 브로드캐스트 표에서 로봇 이름 앞에 🗼 아이콘이 붙은 행은 **관제가 위치를
계산하지 않는 로봇**입니다. Gazebo에서 돌아가는 Pinky(`PK-xx`)가 여기에 해당합니다.

| | 시뮬레이션 로봇 `RS-xx` | 현장 로봇 `PK-xx` |
|---|---|---|
| 위치·방위 | 관제가 계산 | **로봇이 실제로 움직인 결과를 보고** |
| 배터리 | 관제가 계산 | **로봇이 주행량·구획 온도로 계산해 보고** |
| 관제가 내리는 것 | (없음) | 경로 웨이포인트 · 안전 정지 · 속도 상한 · 충전 |
| 배차·FEFO·안전 판단 | 동일 | 동일 |

- 아이콘이 **회색(연결 끊김)** 이면 그 로봇은 `예비 대기`로 내려가 배차에서
  제외됩니다. 진행 중이던 태스크는 자동으로 회수되어 다른 로봇에 재할당됩니다.
- 재접속하면 별도 조작 없이 운영에 복귀합니다.
- 현장 로봇이 관제가 모르는 장애물 앞에서 멈추면, 상세 패널 **안전 메모**에
  `라이다 전방 0.12 m 장애물 — 전진 정지`처럼 사유가 표시되고 10초 이상
  지속되면 운행 이력에도 남습니다.

시뮬레이터 실행 방법은
[robo_pinky 사용 설명서](../../robo_pinky/docs/USER_GUIDE.md)를 보세요.
...
- **스텝 진행** — 각 단계가 어디까지 끝났는지 체크 표시. 출고는 6단계다.

```
① C-01-09 랙으로 이동   ② 집품   ③ LOAD-C 적재 스테이션으로 이동
④ LOAD-C 로봇팔이 적재   ⑤ OUT-1 출고 도크로 이동   ⑥ 하역 및 검수
```

③④가 **적재 스테이션** 단계다. 구획마다 로봇팔이 한 대 있어, 랙에서 집품한
화물을 이 자리에서 로봇 위에 실어 준다. 현장 로봇팔이 접속해 있으면 관제가
팔에 적재를 지시하고 팔의 완료 보고를 기다리며, 팔이 없으면 다른 단계처럼
시간으로 진행한다(운영은 멈추지 않는다).
...
**Q. 실제 로봇에 연결할 수 있나요?**
관제센터는 기동할 때 로봇 링크 게이트웨이(TCP 8788)를 함께 엽니다. 여기에 접속한
로봇은 위치·배터리를 스스로 보고하고, 관제는 경로·정지·속도 상한만 내려보냅니다.
[robo_pinky](../../robo_pinky/)가 이 규약으로 Gazebo의 Pinky를 붙인 참고 구현이며,
같은 프로토콜을 쓰면 실 장비도 그대로 붙습니다.

**Q. 현장 로봇이 접속했는데 일을 안 합니다.**
1. 상태 브로드캐스트 표의 🗼 아이콘 색을 보세요. 회색이면 링크가 끊긴 상태입니다.
2. 상세 패널 **안전 메모**를 확인하세요. `보호 필드`·`장애물 정지`면 안전 장치가
   정상 작동 중입니다.
3. `전체 비상정지`가 걸려 있지 않은지 확인하세요. 비상정지는 현장 로봇에도
   그대로 전달됩니다.
